...
public class BandPassPitchDetect extends AbstractRenderCommand<IAudioRenderTarget> {
	...
	protected void run(IAudioRenderTarget target) throws RenderCommandException {
		double meanNoteBefore = 0;
		double maxMean = 0;
		double meanNoteAfter = 0;
		int maxMeanBand = -1;
		for (int i = 0; i < bandPassFilterBank.filteredSamples.length; ++i) {
			double mean = getMean(i);
			if (mean > maxMean) {
				meanNoteBefore = (i > 0 ? getMean(i - 1) : 0);
				maxMean = mean;
				meanNoteAfter = (i < bandPassFilterBank.filteredSamples.length - 1 ? getMean(i + 1) : 0);
				maxMeanBand = i;
			}
		}
		int pitch = maxMeanBand + bandPassFilterBank.lowestNote;
		//System.out.println(target.getTotalElapsedFrames());
		System.out.println("PD pitch=" + pitch + ", meanLast=" + meanNoteBefore + ", mean=" + maxMean + ", meanNext=" + meanNoteAfter);
		lastNote = pitch;
	}
